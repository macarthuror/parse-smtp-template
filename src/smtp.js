 ...
 *
 * Some of the variables have a error label of "is never read" but is because how are they use it in the template
 ...
 */
const SmtpMailAdapter = (mailOptions) => {
    console.log("-------------------\n" + mailOptions)
    ...
    const _templates = mailOptions.template || false
    const _templatePath = mailOptions.templatePath || ""
    const _multiTemplate = mailOptions.multiTemplate || false
    const _multiLang = mailOptions.multiLang || false
    ...
    const sendMail = mail => {
        ...
        const appName = mail.subject.split("for ")[1] // eslint-disable-line
        const username = mail.text.split("username=")[1] // eslint-disable-line
        var filePath = ""
        var template = ""
        const confirmOptions = mailOptions.confirmOptions || {}
        const passwordOptions = mailOptions.passwordOptions || {}

        const subject = mail.subject.indexOf("Password") !=-1
            ? passwordOptions.subject || mail.subject
            : confirmOptions.subject || mail.subject

        const body = mail.subject.indexOf("Password") !=-1 // eslint-disable-line
        ...
        const btn = mail.subject.indexOf("Password") !=-1 // eslint-disable-line
            ? passwordOptions.btn || "Reset Password"
            : confirmOptions.btn || "Confirm Email"

        const options = mail.subject.indexOf("Password") !=-1 // eslint-disable-line
    ...
    var sendVerificationEmail = data => {
        ...
        const link = data.link // eslint-disable-line
        const appName = data.appName // eslint-disable-line
        const defOptions = mailOptions.confirmOptions
        const options = mailOptions.passwordOptions.others || {} // eslint-disable-line
        ...
        let body = (_multiLang && typeof langOptions !== "undefined") // eslint-disable-line
            ? langOptions.body
            : defOptions.body

        let btn = (_multiLang && typeof langOptions !== "undefined") // eslint-disable-line
            ? langOptions.btn
            : defOptions.btn
